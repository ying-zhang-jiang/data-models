...
class Base(object):
    ...
    def _set_value(self, name, value):
        ...
        """
        try:
            if name in self.YANG_PROPERTY_MAP.keys():
                self._properties[self.YANG_PROPERTY_MAP[name]] = value
            elif name in self.YANG_PROPERTY_MAP.values():
    ...
    def _create(self, locals_dict):
        ...
        response = self._transport._create(self._parent, yang=self.__class__, url=relative_url, locals_dict=locals_dict)
        self._set_properties(response)
        return self

    def _read(self, key_value=None):
        relative_url = self._build_relative_url(self.YANG_NAME, key_value)
        response = self._transport._read(self._parent, yang=self.__class__, url=relative_url)
    ...
    def _update(self, locals_dict):
        ...
        self._transport._update(self._parent, yang=self.__class__, url=relative_url, locals_dict=locals_dict)
    ...
    def _delete(self):
        for i in range(len(self)):
            ...
            self._transport._delete(self._parent, yang=self.__class__, url=relative_url)
        self._clear()

    def _execute(self, rpc_or_action, method, payload = None):
        if self.YANG_KEY is not None:
            relative_url = self._build_relative_url(self.YANG_NAME, self._get_value(self.YANG_KEY))
        else:
            relative_url = self._build_relative_url(self.YANG_NAME, None)
        relative_url = '%s/%s' % (relative_url, method)
        return self._transport._execute(self._parent, rpc_or_action, yang=self.__class__, url=relative_url, locals_dict=payload)
